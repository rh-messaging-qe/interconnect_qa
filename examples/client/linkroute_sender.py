...
class SenderMessageHandler(MessagingHandler):
  ...
  def on_start(self, event):
    ...
    for x in xrange(self.sender_cnt):
      sender = event.container.create_sender(self.conn, '%s.%s' % (self.DESTINATION, x))
  ...
  def on_sendable(self, event):
    if event.sender:
      ...
      if sender.msg_cnt < self.max_messages:
        msg = Message(body="Hello World", properties={'seq': sender.msg_cnt})
  ...
  def on_connection_closed(self, event):
    ...
    for x in self.senders.itervalues(): pprint.pprint("address %s, msg_count: %s" % (x.address_name, x.msg_cnt))
    if self.error:
      print("Test Failed")
    else:
